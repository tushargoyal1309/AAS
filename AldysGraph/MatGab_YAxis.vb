...
Namespace AldysGraph
	...
	Public Class YAxis
		...
		Private m_upperAlarmLine As Boolean
		Private m_upperAlarmLineColor As Color
		Private m_upperAlarmLineY As Double
		Private m_lowerAlarmLine As Boolean
		Private m_lowerAlarmLineColor As Color
		Private m_lowerAlarmLineY As Double

		Public yArrSteps As ArrayList
		#end region
		#region " Constructors"
		''' <summary>
		...
		Public Sub New()
			...
			Me.m_upperAlarmLineColor = InlineAssignHelper(Me.m_lowerAlarmLineColor, Me.m_color)
			...
			'-------------------------------------------------------------
			Me.Title = m_title
			Me.IsVisible = Defaults.YAxis.IsVisible
			Me.ScaleFontSpec.Angle = 90F
			Me.TitleFontSpec.Angle = -180F
			'-------------------------------------------------------------
			Me.yArrSteps = New ArrayList()
		End Sub
		Public Sub New(rhs As YAxis)
			m_min = rhs.Min
			m_max = rhs.Max
			m_step = rhs.[Step]
			...
			Me.m_upperAlarmLine = rhs.upperAlarmLine
			Me.m_upperAlarmLineColor = rhs.upperAlarmLineColor
			Me.m_upperAlarmLineY = rhs.upperAlarmLineY
			Me.m_lowerAlarmLine = rhs.lowerAlarmLine
			Me.m_lowerAlarmLineColor = rhs.lowerAlarmLineColor

			Me.m_lowerAlarmLineY = rhs.lowerAlarmLineY
		End Sub
		''' <summary>
		...
		'			return new YAxis();  
		'		}
		Public Property UpperAlarmLine() As Boolean
			Get
				Return m_upperAlarmLine
			End Get
			Set
				m_upperAlarmLine = value
			End Set
		End Property
		Public Property UpperAlarmLineColor() As Color
			Get
				Return m_upperAlarmLineColor
			End Get
			Set
				m_upperAlarmLineColor = value
			End Set
		End Property
		Public Property UpperAlarmLineY() As Double
			Get
				Return m_upperAlarmLineY
			End Get
			Set
				m_upperAlarmLineY = value
			End Set
		End Property
		Public Property LowerAlarmLine() As Boolean
			Get
				Return m_lowerAlarmLine
			End Get
			Set
				m_lowerAlarmLine = value
			End Set
		End Property
		Public Property LowerAlarmLineColor() As Color
			Get
				Return m_lowerAlarmLineColor
			End Get
			Set
				m_lowerAlarmLineColor = value
			End Set
		End Property
		Public Property LowerAlarmLineY() As Double
			Get
				Return m_lowerAlarmLineY
			End Get
			Set
				m_lowerAlarmLineY = value
			End Set
		End Property
		...
		Public Function TransformRev(x As Single) As Double
			...
			Else
				Return DirectCast(Me.maxScale - (Me.maxScale - Me.minScale) * ratio, Double)
			End If
		End Function
		...
		Public Sub SetupScaleData(pane As AldysPane)
			...
			Me.minPix = pane.AxisRect.Top
			Me.maxPix = pane.AxisRect.Bottom
		...
		Public Function CalcSpace(g As Graphics, pane As AldysPane, scaleFactor As Double) As Single
			...
			If pane.XAxis.IsVisible Then
				Dim tmpSpace As Single = pane.XAxis.GetScaleMaxSpace(g, pane, scaleFactor).Width / 2F + charHeight
		...
		Public Sub DrawScale(g As Graphics, pane As AldysPane, scaleFactor As Double)
			...
			'Draw alram Lines
			If Me.m_upperAlarmLine Then
				pen.Color = Me.m_upperAlarmLineColor
				Dim nY As Single = Me.LocalTransform(Me.m_upperAlarmLineY)
				If Me.m_upperAlarmLineY <= Me.Max AndAlso Me.m_upperAlarmLineY >= Me.Min Then
					g.DrawLine(pen, nY, 0F, nY, topPix)
				End If
			End If
			If Me.m_lowerAlarmLine Then
				pen.Color = Me.m_lowerAlarmLineColor
				Dim nY As Single = Me.LocalTransform(Me.m_lowerAlarmLineY)
				If Me.m_lowerAlarmLineY <= Me.Max AndAlso Me.m_lowerAlarmLineY >= Me.Min Then
					g.DrawLine(pen, nY, 0F, nY, topPix)
				End If
			End If
		...
		Public Function ReverseTransform(pixVal As Single) As Double
			...
			val = DirectCast(pixVal - Me.minPix, Double) / DirectCast(Me.maxPix - Me.minPix, Double) * (Me.maxScale - Me.minScale) + Me.minScale
		...
		Public Function LocalTransform(x As Double) As Single
			...
			rv = DirectCast((Me.maxPix - Me.minPix) * (1F - ratio), Single)
			'			else
		...
		Public Sub ResetAutoScale(pane As AldysPane)
			Me.m_minAuto = True
			Me.m_maxAuto = True
			Me.m_stepAuto = True
			'pane.AxisChange();
		...
		Public Function MakeDefaultArray() As ArrayList
			...
			If Me.IsText AndAlso not (Me.TextLabels  is nothing)  Then
