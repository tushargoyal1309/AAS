...
namespace AldysGraph
{
	...
	public class YAxis
	{
		...
		private FontSpecs	titleFontSpec,
							scaleFontSpec;
		...
		private bool upperAlarmLine;
		private Color upperAlarmLineColor;
		private double upperAlarmLineY;
		private bool lowerAlarmLine;
		private Color lowerAlarmLineColor;
		private double lowerAlarmLineY;

		public ArrayList yArrSteps;
		#endregion
		#region Constructors
		/// <summary>
		...
		public YAxis()
		{
			...
			this.ticSize = Defaults.Axis.TicSize;
			this.minorTicSize = Defaults.Axis.MinorTicSize;
			this.gridDashOn = Defaults.Axis.GridDashOn;
			this.gridDashOff = Defaults.Axis.GridDashOff;
			this.gridPenWidth = Defaults.Axis.GridPenWidth;
		
			this.isVisible = true;
			this.isShowGrid = Defaults.Axis.IsShowGrid;
			this.isReverse = Defaults.Axis.IsReverse;
			this.isOmitMag = false;
			this.isTic = Defaults.Axis.IsTic;
			this.isInsideTic = Defaults.Axis.IsInsideTic;
			this.isOppositeTic = Defaults.Axis.IsOppositeTic;
			this.isMinorTic = Defaults.Axis.IsMinorTic;
			this.isMinorInsideTic = Defaults.Axis.IsMinorInsideTic;
			this.isMinorOppositeTic = Defaults.Axis.IsMinorOppositeTic;
		
			this.type = Defaults.Axis.Type;
			this.title = "";
			...
			this.ticPenWidth = Defaults.Axis.TicPenWidth;
			this.color = Defaults.Axis.Color;
			this.upperAlarmLineColor = this.lowerAlarmLineColor = this.color;
			this.gridColor = Defaults.Axis.GridColor;
			
			this.titleFontSpec = new FontSpecs(
				Defaults.Axis.TitleFontFamily, Defaults.Axis.TitleFontSize,
				Defaults.Axis.TitleFontColor, Defaults.Axis.TitleFontBold,
				Defaults.Axis.TitleFontUnderline, Defaults.Axis.TitleFontItalic );
			this.titleFontSpec.IsFilled = false;
			this.titleFontSpec.IsFramed = false;

			this.scaleFontSpec = new FontSpecs(
				Defaults.Axis.ScaleFontFamily, Defaults.Axis.ScaleFontSize,
				Defaults.Axis.ScaleFontColor, Defaults.Axis.ScaleFontBold,
				Defaults.Axis.ScaleFontUnderline, Defaults.Axis.ScaleFontItalic );
			this.scaleFontSpec.IsFilled = false;
			this.scaleFontSpec.IsFramed = false;
			//-------------------------------------------------------------
			this.Title = title;
			this.IsVisible = Defaults.YAxis.IsVisible;
			this.ScaleFontSpec.Angle = 90.0F;
			this.TitleFontSpec.Angle = -180F;
			//-------------------------------------------------------------
			this.yArrSteps = new ArrayList();
		}
		public YAxis( YAxis rhs ) 
		{
			...
			titleFontSpec = new FontSpecs( rhs.TitleFontSpec );
			scaleFontSpec = new FontSpecs( rhs.ScaleFontSpec );
			...
			yArrSteps = rhs.yArrSteps;
			this.upperAlarmLine = rhs.upperAlarmLine;
			this.upperAlarmLineColor = rhs.upperAlarmLineColor;
			this.upperAlarmLineY = rhs.upperAlarmLineY;
			this.lowerAlarmLine = rhs.lowerAlarmLine;
			this.lowerAlarmLineColor = rhs.lowerAlarmLineColor;
			this.lowerAlarmLineY = rhs.lowerAlarmLineY;
...
//			return new YAxis();  
//		}
		public bool UpperAlarmLine
		{
			get { return upperAlarmLine; }
			set { upperAlarmLine = value; }
		}
		public Color UpperAlarmLineColor
		{
			get { return upperAlarmLineColor; }
			set { upperAlarmLineColor = value; }
		}
		public double UpperAlarmLineY
		{
			get { return upperAlarmLineY; }
			set { upperAlarmLineY = value; }
		}
		public bool LowerAlarmLine
		{
			get { return lowerAlarmLine; }
			set { lowerAlarmLine = value; }
		}
		public Color LowerAlarmLineColor
		{
			get { return lowerAlarmLineColor; }
			set { lowerAlarmLineColor = value; }
		}
		public double LowerAlarmLineY
		{
			get { return lowerAlarmLineY; }
			set { lowerAlarmLineY = value; }
		}
		...
		/// the scale values
		/// </summary>
		public FontSpecs ScaleFontSpec
		{
		...
		/// </summary>
		public FontSpecs TitleFontSpec
		{
		...
		public double TransformRev( float x )
		{
			// Must take into account Log, and Reverse Axes
			double ratio;
			ratio=1;
			if ( this.IsLog )
				...
				ratio = ( x - this.minPix ) / ( this.maxPix - this.minPix );
				
			if ( this.isReverse )
				return (double) ( this.minScale + ( this.maxScale - this.minScale ) * ratio );
			else
				return (double) ( this.maxScale - ( this.maxScale - this.minScale ) * ratio );
		}
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public void DrawTitle( Graphics g, AldysPane pane, double scaleFactor )
		{
			string str;
		...
		public void SetupScaleData( AldysPane pane )
		{
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public void SetTransformMatrix( Graphics g, AldysPane pane, double scaleFactor )
		{
		...
		public SizeF GetScaleMaxSpace( Graphics g, AldysPane pane, double scaleFactor )
		{
			string tmpStr;
			double	dVal,
		...
		public float CalcSpace( Graphics g, AldysPane pane, double scaleFactor )
		{
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public void DrawScale( Graphics g, AldysPane pane, double scaleFactor )
		{
			float	rightPix,
				topPix;
			...
			//Draw alram Lines
			if(this.upperAlarmLine)
			{
				pen.Color = this.upperAlarmLineColor;				
				float nY = this.LocalTransform(this.upperAlarmLineY);				
				if(this.upperAlarmLineY <= this.Max && this.upperAlarmLineY >= this.Min )
				 g.DrawLine( pen, nY, 0.0F, nY,topPix );			
			}
			if(this.lowerAlarmLine)
			{
				pen.Color = this.lowerAlarmLineColor;				
				float nY = this.LocalTransform(this.lowerAlarmLineY);				
				if(this.lowerAlarmLineY <= this.Max && this.lowerAlarmLineY >= this.Min )
					g.DrawLine( pen, nY, 0.0F, nY,topPix );			
			}
		...
		public void PickScale( double minVal, double maxVal )
		{
			...
			else if ( this.IsDate )			// Date Scale
			{
				...
				if ( this.stepAuto )
					this.step = CalcDateStepSize( this.max - this.min, Defaults.Axis.TargetSteps );
			...
			else			// Linear Scale
			{
				...
				if ( this.minAuto && this.min > 0 &&
					this.min / ( this.max - this.min ) < Defaults.Axis.ZeroLever )
					this.min = 0;
				...
				if ( this.maxAuto && this.max < 0 &&
					...
					Defaults.Axis.ZeroLever )
					this.max = 0;
		
				// Calculate the new step size
				if ( this.stepAuto )
					this.step = CalcStepSize( this.max - this.min, Defaults.Axis.TargetSteps );
		
				// Calculate the new step size
				if ( this.minorStepAuto )
					this.minorStep = CalcStepSize( this.step, Defaults.Axis.TargetMinorSteps );
		...
		protected double CalcDateStepSize( double range, double targetSteps )
		{
			...
			if ( range > Defaults.Axis.RangeYearYear )
			{
			...
			else if ( range > Defaults.Axis.RangeYearMonth )
			{
				majorUnit = DateUnit.Year;
			...
			else if ( range > Defaults.Axis.RangeMonthMonth )
			{
			...
			else if ( range > Defaults.Axis.RangeDayDay )
			{
			...
			else if ( range > Defaults.Axis.RangeDayHour )
			{
				majorUnit = DateUnit.Day;
			...
			else if ( range > Defaults.Axis.RangeHourHour )
			{
			...
			else if ( range > Defaults.Axis.RangeHourMinute )
			{
				majorUnit = DateUnit.Hour;
				tempStep = 1.0;
				scaleFormat = "&hh:&nn";
			...
			else if ( range > Defaults.Axis.RangeMinuteMinute )
			{
			...
			else if ( range > Defaults.Axis.RangeMinuteSecond )
			{
				majorUnit = DateUnit.Minute;
				tempStep = 1.0;
				scaleFormat = "&nn:&ss";
		...
		/// the graph is drawn (i.e., <see cref="AldysPane.Draw"/> is called).
		/// </summary>
...
//			if ( ( this.isReverse && !(YAxis) ) ||
//				( !this.isReverse && (YAxis ) ) )
...
//				rv = (float) ( ( this.maxPix - this.minPix ) * ratio );
		...
		private void MakeLabel( int index, double dVal, out string label )
		{
			...
			else if ( this.IsDate )
			{
				if ( this.scaleFormat == null )
					this.scaleFormat = Defaults.Axis.ScaleFormat;
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public void DrawLabels( Graphics g, AldysPane pane, double baseVal, int nTics,
		...
		public void ResetAutoScale( AldysPane  pane )
		{
			this.minAuto = true;
			this.maxAuto = true;
			this.stepAuto = true;
